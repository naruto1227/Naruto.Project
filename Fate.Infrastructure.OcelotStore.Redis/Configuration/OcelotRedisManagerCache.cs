...
namespace Fate.Infrastructure.OcelotStore.Redis
{
    /// <summary>
    /// 张海波
    /// 2019.08.13
    ...
    public class OcelotRedisManagerCache<T> : IOcelotCache<T> where T : class
    {
        ...
        public OcelotRedisManagerCache(IRedisOperationHelp _redis)
        {
            redis = _redis;
        }
        ...
        public void AddAndDelete(string key, T value, TimeSpan ttl, string region)
        {
            ...
            if (redis.RedisKey().Exists(prefix + key))
            {
                redis.RedisKey().Remove(prefix + key);
            }
        ...
        public T Get(string key, string region)
        {
            ...
            return redis.RedisString().Get<T>(prefix + key);
        }
    }
}

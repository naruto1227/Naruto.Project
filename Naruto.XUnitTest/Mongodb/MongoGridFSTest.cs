...
namespace Naruto.XUnitTest.Mongodb
{
    ...
    public class MongoGridFSTest
    {
        ...
        {
            //获取需要上传的资源
            var bytes = File.ReadAllBytes(Path.Combine(@"D:\自有项目\Naruto.DDD\Naruto.XUnitTest", "Mongodb", "MongoCommandTest.cs"));
        ...
        {
            using var fileStream = new FileStream(Path.Combine(@"D:\自有项目\Naruto.DDD\Naruto.XUnitTest", "Mongodb", "MongoCommandTest.cs"), FileMode.Open, FileAccess.Read);
        ...
        {
            ObjectId.Parse("123456789012345678901234");
            var o = ObjectId.GenerateNewId();
            var o2 = ObjectId.Empty;
        }
    }
}
